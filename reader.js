...
function renderCurrent(animate = false, opts = {}) {
    ...
    // 说话人槽位：<say> 放角色名，<cg> 放 img 标题——同一处、同字形。
    setSpeaker(frag.kind === 'say' ? (frag.speaker || '') : cgTitle(frag));
...
function renderText(frag, animate = false, chatIndex) {
    ...
    if (animate && getSetting('typewriter') && !hasHtmlBlock) {
...
/** `<cg img="标题">正文</cg>` 的 img 属性 → 说话人槽位（面板左上角，与 <say> 的角色名同一处、同字形）。
 *  正文不做特殊处理，和 <narration> 一样走普通叙述文本，照常打字机。
...
function renderLiveFragment(lastFrag, parsed) {
    ...
    if (isPlain) {
        ...
        // 标题只在 </cg> 收尾、PAIR_RE 匹配成片段之后才出得来，流式中途槽位是空的属于正常表现。
        setSpeaker(lastFrag.kind === 'say' ? (lastFrag.speaker || '') : cgTitle(lastFrag));
