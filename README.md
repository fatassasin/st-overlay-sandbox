...
`test-assets/` 里那六张图（测试面板「填入内置测试图」用的背景／立绘／道具）是 AI 生成的，**不在上述 CC BY-NC 授权范围内**，随仓库附带只为让测试面板开箱即用。

多数司法辖区（含美国）认为缺少人类作者的纯 AI 生成图像不构成受版权保护的作品，因此本项目不对它们主张任何版权，也不限制你怎么用。它们只是占位素材，不是作品——需要正式素材的话请自备。
