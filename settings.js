...
export const DEFAULT_SETTINGS = {
    ...
    injectProtocol: false,  // 把舞台协议提示词注入 AI 上下文（教 AI 用标签输出）
    ...
    fontFamily: 'serif',    // 'serif' | 'sans' | 'mono' | 'cn-serif' | 'kai' | 'rounded'
    fontSize: 17,           // px，正文字号 → --ov-font-size
    lineHeight: 1.8,        // 行高 → --ov-line-height
    spacing: 'cozy',        // 界面间距预设 'compact' | 'cozy' | 'roomy' → 内距/片段间距
    panelWidth: 70,         // 正文面板宽度，占屏幕宽度百分比 → --ov-panel-width
    ...
    followReplyColor: true,     // 顶部「我的输入」回显跟随 SillyTavern 正文色（与「正文」同一变量）
    quoteColor: '',         // 中英文引号自定义色
    textColor: '',          // 正文自定义色
    emphasisColor: '',      // Markdown 粗体自定义色
    bracketColor: '',       // 括号段自定义色
    italicColor: '',        // Markdown 斜体自定义色
    replyColor: '',         // 顶部「我的输入」回显自定义色
    textBgEnabled: false,   // 文本背景总开关（开且当前有生成背景图时才显示）
    textBgColor: '#000000', // 文本背景色（每条文本块衬底）→ --ov-text-bg
    textBgOpacity: 45,      // 文本背景不透明度 0..100（需 textBgEnabled + 有 bg 图）
    ...
    backgroundGlowBrightness: 100, // 背景泛光亮度 %
    notify: 'toast',        // 通知方式 'toast' | 'inline' | 'off'
    typewriter: true,       // 打字机逐字揭示（非流式时生效；流式自动让位）
    typewriterSpeed: 53,    // 打字速度滑条·普通正文（越小越慢；内部换算 151-value = ms/字）
    typewriterSpeedVn: 53,  // 打字速度滑条·VN 楼层（同上换算）

    // —— 导航 / 界面行为 ——
    showFloorMeta: true,    // 顶栏左上角显示楼层号（酒馆 mesid）
    wheelStrength: 640,     // 跨楼层滚轮力度阈值（越大越「重」）；片段阈值按比例派生
    composerAutohide: true, // 输入框默认隐藏，鼠标移到底部热区/聚焦时显示
    composerHotzone: 28,    // 输入框唤起热区高度 px（鼠标下移到距底部多少 px 才唤起；越大越易唤起）
    composerArrow: true,    // 底部显示小箭头提示「鼠标下移唤起输入框」
    composerArrowSize: 28,  // 底部提示箭头宽度 px → --ov-arrow-size
    composerArrowHeight: 12, // 底部提示箭头形状高度 px → --ov-arrow-height（扁长可调）
    composerArrowBottom: 6,  // 底部提示箭头离屏幕底部距离 px → --ov-arrow-bottom
    ...
    replyHotzone: 28,       // 顶部唤起热区高度 px → --ov-reply-hotzone
    ...
    replyTop: 52,           // 回显胶囊离屏幕顶部距离 px → --ov-reply-top（默认让开顶栏楼层号与图标）
    replyMaxHeight: 30,     // 回显胶囊正文最大高度 vh，超出内部滚动 → --ov-reply-maxh
    topFade: 0,             // 正文顶部虚化高度 vh：0=不虚化 → --ov-top-fade
    topbarAutohide: false,  // 右上角图标默认常驻；用户需要时可在设置里折叠
    jumpbarAutohide: false, // 右侧楼层按钮默认常驻；开启后悬停右侧热区显示
    jumpbarInset: 14,       // 右侧楼层滑动条距右边缘 px → --ov-jumpbar-inset
    idleDim: false,         // 无操作自动黑屏（鼠标/键盘静止到时长后 3s 渐黑，动一下瞬间还原）
    idleDimDelay: 60,       // 无操作多少秒后开始渐黑
    bottomFade: 0,          // 底部黑框高度（vh 单位）：0=完全无黑框；>0 控制正文面板离底部多远 + 虚化遮罩高度
    topTextHeight: 40,      // 顶部文本高度 px：正文第一行从屏幕顶部向下的偏移（框定文字起始位置）→ --ov-text-top
    vnTextHeight: 56,       // 视觉小说正文显示区高度（vh 占屏）→ --ov-panel-maxh（固定高度、内部滚动）
    plainTextMaxHeight: 56, // 兼容旧 key：底部文本高度（vh），0=无额外底部留白 → --ov-plain-bottom-gap
    pointerNavigation: true, // 左右键导航开关：开=保持当前切楼/切片；关=左右键不导航
    audioEnabled: true,     // 音频标签（bgm/sfx/voice）总开关
    audioVolume: 80,        // 音频总音量 %
    fullscreen: false,      // 记忆上次全屏开关，open() 时恢复
    thinkingLine: true,     // 思维链顶部横条
    keyButtonEnabled: true, // 显示可移动虚拟按键
    keyButtonCode: 'AltRight',
    keyButtonKey: 'Alt',
    keyButtonDocked: false, // 放到输入框发送键左侧
};
